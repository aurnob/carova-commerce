...
export const addItemOptimistically = (
  ...
  const currentCart =
    optimisticCart ||
    queryClient.getQueryData<HttpTypes.StoreCart | null>(
  ...
  const newItemSubtotal = updatedItems.reduce(
...
export const updateLineItemOptimistically = (
  ...
  const optimisticCart: OptimisticCart = {
    ...currentCart,
    items: updatedItems,
    item_subtotal: updatedItems.reduce(
      (sum, item) => sum + (item.total || 0),
      0
    ),
    isOptimistic: true,
...
export const createOptimisticCart = (
