...
export const getPricePercentageDiff = (
  original: number,
  calculated: number
): string => {
...
export const getPricesForVariant = (
  variant: VariantWithCalculatedPrice | null | undefined
): {
...
export const getPriceFilterOptions = (currency_code: string) => {
  ...
  const symbol = formatPrice({ amount: 0, currency_code })
    .replace(/[\d.,]/g, "")
    .trim()
  ...
  if (currencyUpper === "DKK") {
    return [
      ...
      {
        id: "350-700",
        label: `${symbol}350 - ${symbol}700`,
        min: 350,
        max: 700,
      },
      {
        id: "700-1050",
        label: `${symbol}700 - ${symbol}1050`,
        min: 700,
        max: 1050,
      },
