...
const CountrySelect = ({ regions, className }: CountrySelectProps) => {
  // Use the router and pathname hooks inside the component
  const router = useRouter()
  const pathname = usePathname()

  // Get the country code from the current path
  const pathCountryCode = getCountryCodeFromPath(pathname)
  const currentPath = pathname.replace(`/${pathCountryCode}`, "") || "/"

  // Mutations for updating and creating the cart
  const updateCartMutation = useUpdateCart()
  const createCartMutation = useCreateCart()

  // Prepare the countries array based on the regions
  ...
  // Get the current country based on the path country code
  ...
  const handleChange = async (countryCode: string) => {
    ...
    // Generate the new path with the selected country's country code
    const newPath = buildPathWithCountryCode(currentPath, option.country_code)

    // Navigate to the new path
    router.push(newPath)

    // If the region changes, update or create the cart
  ...
  return (
    <Select value={pathCountryCode || ""} onValueChange={handleChange}>
      <SelectTrigger
        variant="minimal"
        className={clsx(
          "text-sm text-white w-full justify-start font-normal",
          className
        )}
      >
